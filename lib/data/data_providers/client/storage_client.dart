...
class StorageClient {
  ...
  StorageClient({required this.config, required this.ref}) {
    ...
    storage_api.ChannelCredentials creds =
        const storage_api.ChannelCredentials.insecure();
    channel = storage_api.ClientChannel(config.hostname,
        port: config.port,
        options: storage_api.ChannelOptions(credentials: creds));
  ...
  Future<storage_api.StandardResponse> destroyDB() async {
    try {
      ...
      debugPrint("Error destroying database: $e");
      rethrow;
    }
  }

  Future<storage_api.StandardResponse> write(
      storage_api.KeyValue keyValue) async {
    try {
  ...
  Future<storage_api.ListResponse> listNodes(
      storage_api.SubtreeInfo subtreeInfo) async {
    try {
